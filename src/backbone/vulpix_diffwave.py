...
from src.utils.vulpix_utils import calc_diffusion_step_embedding
...
class Residual_block(nn.Module):
    def __init__(self, res_channels, skip_channels, dilation, 
        ...
        self.cond_conv = Conv(in_channels, 2*self.res_channels, kernel_size=1)  # 80 is mel bands
...
class DiffWaveImputer(nn.Module):
    def __init__(self, in_channels=None, res_channels=64, skip_channels=64, out_channels=None,
                 num_res_layers=12, dilation_cycle=4,
        ...
        # FCST trainers inject seq metadata; allow it and infer channels when omitted.
        if in_channels is None:
            in_channels = seq_channels
        if out_channels is None:
            out_channels = in_channels
        if in_channels is None or out_channels is None:
            raise ValueError("DiffWaveImputer requires in_channels/out_channels or seq_channels.")

        self.seq_channels = in_channels
    ...
    def forward(self, input_data):
        ...
        #conditional = conditional * mask legacy method
